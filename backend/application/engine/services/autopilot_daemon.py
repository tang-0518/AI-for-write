        ...
        if beats:
            for i, beat in enumerate(beats):
                ...
                if use_wf:
                    ...
                    # 系数 2.5：1 字≈1 token + thinking tokens 开销预留
                    max_tokens = int(beat.target_words * 2.5)
            ...
            if use_wf:
                ...
                cfg = GenerationConfig(max_tokens=6000, temperature=0.85)
                beat_content = await self._stream_llm_with_stop_watch(prompt, cfg, novel=novel)
            else:
                beat_content = await self._stream_one_beat(
...
5. 不要写章节标题"""
        ...
        max_tokens = int(beat.target_words * 2.5) if beat else 6000
