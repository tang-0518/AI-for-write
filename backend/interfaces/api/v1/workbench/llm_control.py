...
def _normalize_model_items(data: Dict[str, Any]) -> List[ModelItem]:
    """将不同网关的 /models 响应统一为 ModelItem 列表（OpenAI 格式）。"""
...
def _normalize_gemini_model_items(data: Dict[str, Any]) -> List[ModelItem]:
    """将 Gemini /models 响应统一为 ModelItem 列表。
    Gemini 返回格式: {"models": [{"name": "models/gemini-2.5-pro", "displayName": "...", ...}]}
    """
    items: List[ModelItem] = []
    ...
    for entry in raw_list:
        ...
        display_name = str(entry.get('displayName', model_id))
        if not model_id:
            continue
        items.append(ModelItem(id=model_id, name=display_name, owned_by='google'))
...
async def list_models(payload: ModelListRequest) -> ModelListResponse:
    """根据当前配置的 endpoint 拉取模型列表（OpenAI / Anthropic / Gemini）。"""
    ...
    try:
        if api_format == 'gemini':
            gemini_base = (base_url or 'https://generativelanguage.googleapis.com/v1beta').rstrip('/')
            url = f'{gemini_base}/models'
            async with httpx.AsyncClient(timeout=timeout) as client:
                response = await client.get(url, params={'key': api_key})
                response.raise_for_status()
                data = response.json()
            normalized = _normalize_gemini_model_items(data)
        ...
        return ModelListResponse(success=True, items=normalized, count=len(normalized))
